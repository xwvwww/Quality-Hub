...
export async function api<T>(path: string, init: RequestInit = {}) {
  ...
  try {
    ...
    throw new Error(
      "Backend недоступен. Проверьте, что API запущен на порту 4000",
    );
  }
  if (response.status === 401 && current) {
    const refreshed = await fetch(
      current.user.systemAdmin
  ...
  if (!response.ok) {
    ...
    const fallback =
      response.status >= 500
        ? `Backend не отвечает корректно (${response.status}) — проверьте порт 4000`
        : `Ошибка запроса ${response.status}`;
    const message = detail || fallback;
    if (typeof window !== "undefined")
      window.dispatchEvent(
        new CustomEvent("quality-hub-toast", {
