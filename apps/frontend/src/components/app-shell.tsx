...
import { BarChart3, Bug, ChevronDown, ClipboardCheck, FileText, FolderKanban, Gauge, LayoutDashboard, LogOut, Moon, PlayCircle, Plug, Settings, ShieldCheck, Sun, Users } from 'lucide-react';
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className={dark?'dark min-h-screen':'min-h-screen'}><div className="flex min-h-screen">
    ...
    <div className="flex-1 min-w-0"><header className="h-16 bg-[var(--panel)] border-b border-[var(--line)] px-7 flex items-center justify-between sticky top-0 z-30 backdrop-blur-xl"><div className="font-semibold">{t.workspace} <ChevronDown className="inline" size={16}/></div><div className="flex gap-2 items-center"><button onClick={theme} className="icon-btn" aria-label="Theme">{dark?<Sun size={19}/>:<Moon size={19}/>}</button><Link href="/profile/settings" className="icon-btn no-underline" aria-label="Настройки" title="Настройки"><Settings size={19}/></Link><Link href="/profile" className="flex items-center gap-2 text-current no-underline rounded-full pr-2 hover:bg-slate-100"><span className="w-9 h-9 bg-gradient-to-br from-indigo-500 to-violet-600 text-white grid place-items-center rounded-full font-bold overflow-hidden shadow-lg shadow-indigo-500/25">{avatar?<img src={avatar} alt={t.profile} className="w-full h-full object-cover"/>:(profile?.firstName?.[0]??'A')+(profile?.lastName?.[0]??'')}</span><span className="hidden lg:block text-sm font-semibold">{profile?`${profile.firstName} ${profile.lastName}`:t.profile}</span></Link></div></header>{children}</div>
