...
export default function ProjectsPage() {
  ...
  const canArchive = canManage || role === "QA_ENGINEER";
  const canDelete = role === "QA_LEAD";
  ...
  async function remove(project: Project) {
    ...
    try {
      ...
      setError(
        reason instanceof Error ? reason.message : "Не удалось удалить проект",
      );
    }
  }
    ...
    <AppShell>
      <main className="p-7 max-w-7xl mx-auto">
        <div className="flex flex-wrap justify-between gap-4 items-end mb-7">
          <div>
            <p className="text-muted text-sm m-0">
              Управление рабочими областями тестирования
            </p>
            <h1 className="text-3xl m-0 mt-1">Проекты</h1>
          </div>
          {canManage && (
            <button
        ...
        <section className="card overflow-hidden">
          <div className="p-4 border-b border-[var(--line)] flex flex-wrap gap-3">
          ...
          {error && !modal && (
            <p className="mx-4 p-3 bg-red-50 text-red-700 rounded-lg">
              {error}
            </p>
          )}
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-sm">
              <thead>
                <tr className="text-left text-muted bg-slate-50/70">
                  <th className="p-4">Проект</th>
                  <th className="p-4">Владелец</th>
                  <th className="p-4">Статус</th>
                  <th className="p-4 text-center">Тест-кейсы</th>
              ...
              <tbody>
                {loading
                  ...
                  : data.items.map((project) => (
                      <tr
                        key={project.id}
                        className="border-t border-[var(--line)] hover:bg-slate-50/50"
                      >
                        <td className="p-4">
                          <div className="flex gap-3 items-center">
                            <span className="w-10 h-10 rounded-lg bg-indigo-50 text-brand grid place-items-center">
                        ...
                        <td className="p-4">
                          <span
                            className={`px-2.5 py-1 rounded-full text-xs font-semibold ${project.status === "ACTIVE" ? "bg-green-50 text-green-700" : "bg-slate-100 text-slate-600"}`}
                          >
                  ...
                  <tr>
                    <td colSpan={6} className="p-14 text-center">
                      ...
                      <p className="text-muted text-sm">
                        Измените параметры поиска или создайте первый проект.
                      </p>
                    </td>
                  </tr>
                )}
          ...
          <div className="p-4 border-t border-[var(--line)] flex justify-between items-center text-sm">
            <span className="text-muted">Всего: {data.meta.total}</span>
        ...
        {modal && (
          <div
            className="fixed inset-0 bg-slate-950/50 grid place-items-center p-4 z-50"
            onMouseDown={(event) => {
              if (event.currentTarget === event.target) setModal(false);
            }}
          >
            <form onSubmit={submit} className="card p-6 w-full max-w-xl">
              <div className="flex justify-between items-start mb-6">
                <div>
                  ...
                  <p className="text-muted text-sm m-0 mt-1">
                    Укажите основные сведения о проекте.
                  </p>
                </div>
                <button
                  type="button"
              ...
              <p className="text-xs text-muted mt-1 mb-4">
                2–12 латинских букв, цифр, _ или -
              </p>
              <label className="block text-sm font-semibold mb-2">
                Описание
              </label>
              <textarea
                ...
                placeholder="Что тестирует команда в этом проекте?"
              />
              {error && (
