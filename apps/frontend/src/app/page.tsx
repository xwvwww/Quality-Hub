...
export default function Login() {
  ...
  async function forgot(event: FormEvent) {
    event.preventDefault(); setForgotLoading(true); setForgotMessage('');
    try { const response=await fetch('/api/auth/forgot-password',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:forgotEmail})});const data=await response.json().catch(()=>null);if(!response.ok)throw new Error(Array.isArray(data?.message)?data.message[0]:data?.message||'Ошибка запроса');setForgotMessage(data.message); }
    catch(reason){setForgotMessage(reason instanceof Error?reason.message:'Не удалось отправить запрос')} finally { setForgotLoading(false); }
  }
  ...
  return <main className="min-h-screen grid lg:grid-cols-2">
    ...
    <section className="flex items-center justify-center p-6"><form onSubmit={submit} className="w-full max-w-md card p-8">
      ...
      <label className="block text-sm font-semibold mb-2">Email</label><input className="field mb-5" type="email" placeholder="name@company.com" autoComplete="email" value={email} onChange={e => setEmail(e.target.value)} required />
      <div className="flex justify-between"><label className="text-sm font-semibold mb-2">Пароль</label><button type="button" onClick={()=>{setForgotEmail(email);setForgotMessage('');setForgotOpen(true)}} className="border-0 bg-transparent text-brand cursor-pointer">Забыли пароль?</button></div>
      <div className="relative"><input className="field pr-12" type={show ? 'text' : 'password'} placeholder="Введите пароль" autoComplete="current-password" value={password} onChange={e => setPassword(e.target.value)} minLength={8} required /><button type="button" aria-label="Показать пароль" onClick={() => setShow(!show)} className="absolute right-3 top-3 border-0 bg-transparent text-muted cursor-pointer">{show ? <EyeOff size={19} /> : <Eye size={19} />}</button></div>
      {error && <p role="alert" className="text-red-600 bg-red-50 p-3 rounded-lg text-sm">{error}</p>}
      <button type="submit" className="btn w-full mt-6" disabled={loading}>{loading ? 'Вход…' : 'Войти'}</button>
    </form></section>
    {forgotOpen&&<div className="fixed inset-0 bg-slate-950/60 z-50 grid place-items-center p-4" onMouseDown={event=>{if(event.target===event.currentTarget)setForgotOpen(false)}}><form onSubmit={forgot} className="card p-7 w-full max-w-md"><h2 className="m-0">Восстановление доступа</h2><p className="text-sm text-muted mb-6">Администратор получит запрос и свяжется с вами.</p><label className="block text-sm font-semibold mb-2">Email</label><input className="field" type="email" required autoFocus value={forgotEmail} onChange={event=>setForgotEmail(event.target.value)} placeholder="name@company.com"/>{forgotMessage&&<p className="p-3 bg-indigo-50 text-brand rounded-lg text-sm">{forgotMessage}</p>}<div className="flex justify-end gap-3 mt-6"><button type="button" className="btn-secondary" onClick={()=>setForgotOpen(false)}>Закрыть</button><button className="btn" disabled={forgotLoading}>{forgotLoading?'Отправка…':'Отправить'}</button></div></form></div>}
