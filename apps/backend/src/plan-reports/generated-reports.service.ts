...
const reportSelect = {
  id: true, testPlanId: true, requestedById: true, status: true, format: true, includeAttachments: true, failedOnly: true,
...
export class GeneratedReportsService {
  ...
  async create(organizationId: string, userId: string, dto: CreateGeneratedReportDto) {
    ...
    const duplicate = await this.prisma.generatedReport.findFirst({ where: { organizationId, testPlanId: dto.testPlanId, requestedById: userId, format: dto.format, failedOnly: dto.failedOnly, status: { in: [ReportJobStatus.QUEUED, ReportJobStatus.PROCESSING] } }, select: { id: true } });
    if (duplicate) throw new BadRequestException('Такой отчёт уже находится в очереди');
    const report = await this.prisma.generatedReport.create({ data: { organizationId, testPlanId: dto.testPlanId, requestedById: userId, format: dto.format, includeAttachments: dto.includeAttachments, failedOnly: dto.failedOnly }, select: reportSelect });
    await this.prisma.auditLog.create({ data: { organizationId, userId, action: 'REPORT_QUEUED', entityType: 'GENERATED_REPORT', entityId: report.id, metadata: { testPlanId: dto.testPlanId, format: dto.format, scope: dto.failedOnly ? 'FAILED_ONLY' : 'ALL' }, ipAddress: RequestContext.ip() } });
    return report;
  }
