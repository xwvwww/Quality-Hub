...
type BaseSnapshot = Awaited<ReturnType<import('./plan-reports.service').PlanReportsService['get']>>;
type ReportSnapshot = Omit<BaseSnapshot, 'cases'> & { cases: Array<BaseSnapshot['cases'][number] & { attachments?: Array<{ fileName:string; dataUrl:string }> }> };
const pdfMake = pdfMakeRuntime as unknown as typeof import('pdfmake');
const statusLabels:Record<string,string>={PASSED:'Успешно',FAILED:'Провалено',BLOCKED:'Заблокировано',SKIPPED:'Пропущено',RETEST:'Ретест',NOT_RUN:'Не выполнено'};
const statusColors:Record<string,string>={PASSED:'#16a34a',FAILED:'#dc2626',BLOCKED:'#d97706',SKIPPED:'#475569',RETEST:'#7c3aed',NOT_RUN:'#64748b'};
...
function duration(seconds:number){const h=Math.floor(seconds/3600),m=Math.floor((seconds%3600)/60),s=seconds%60;return[h&&`${h}ч`,m&&`${m}м`,(s||(!h&&!m))&&`${s}с`].filter(Boolean).join(' ');}
function section(name:string){return name==='PRECONDITION'?'Предусловия':name==='POSTCONDITION'?'Постусловия':'Шаги';}

@Injectable()
export class ReportDocumentBuilder {
  private readonly fonts={Roboto:{normal:require.resolve('pdfmake/fonts/Roboto/Roboto-Regular.ttf'),bold:require.resolve('pdfmake/fonts/Roboto/Roboto-Medium.ttf'),italics:require.resolve('pdfmake/fonts/Roboto/Roboto-Italic.ttf'),bolditalics:require.resolve('pdfmake/fonts/Roboto/Roboto-MediumItalic.ttf')}};
  ...
  async pdf(snapshot:ReportSnapshot):Promise<Buffer>{
    ...
    const doc:TDocumentDefinitions={
      ...
      header:()=>({text:'Quality Hub — Almen Alnur',alignment:'center',fontSize:7,color:'#475569',margin:[0,17,0,0]}),
      footer:(page,pages)=>({text:`Quality Hub · Almen Alnur · ${page} / ${pages}`,alignment:'center',fontSize:7,color:'#64748b'}),
      content:[
        {table:{widths:['*'],body:[[{stack:[{text:'ОТЧЁТ О ТЕСТИРОВАНИИ',bold:true,color:'#6366f1',fontSize:9,characterSpacing:1.2},{text:snapshot.plan.name,fontSize:25,bold:true,margin:[0,5,0,5]},{text:`${snapshot.plan.project.code} · ${snapshot.plan.project.name}`,color:'#64748b'},{columns:[{text:`Версия / Build\n${snapshot.plan.version||'—'} / ${snapshot.plan.build||'—'}`},{text:`Окружение\n${snapshot.plan.environment||'—'}`},{text:`Тест-ран\n${snapshot.run?.name||'Не запускался'}`},{text:`Сформирован\n${new Date(snapshot.generatedAt).toLocaleString('ru-RU')}`}],columnGap:14,fontSize:8,color:'#475569',margin:[0,18,0,0]}],fillColor:'#f5f7ff',margin:[18,16,18,16]}]]},layout:{hLineColor:()=> '#cfd8ea',vLineColor:()=> '#cfd8ea',hLineWidth:()=>1,vLineWidth:()=>1},margin:[0,0,0,12]},
        {columns:[metric('Выполнение',`${m.progress}%`,`${m.executed} из ${m.total}`),metric('Успешность',`${m.passRate}%`,`${m.PASSED} успешно`),metric('Затрачено',duration(m.actualDuration),`Оценка ${duration(m.estimatedDuration)}`),metric('Дефекты',String(m.defects),`${m.FAILED} провалено`)],columnGap:9,margin:[0,0,0,12]},
        {table:{widths:['*','*','*','*','*','*'],body:[Object.keys(statusLabels).map((key)=>({stack:[{text:String(m[key as keyof typeof m]??0),fontSize:17,bold:true,color:statusColors[key]},{text:statusLabels[key],fontSize:7,color:statusColors[key]}],fillColor:key==='FAILED'?'#fff1f2':key==='PASSED'?'#f0fdf4':'#f8fafc',margin:[8,9,8,9]}))]},layout:{hLineColor:()=> '#e2e8f0',vLineColor:()=> '#e2e8f0',hLineWidth:()=>1,vLineWidth:()=>1},margin:[0,0,0,16]},
        {text:'Подробные результаты тест-кейсов',fontSize:14,bold:true,margin:[0,0,0,7]},...caseBlocks,
      ],
    };
    const allowedFonts=new Set(Object.values(this.fonts.Roboto));pdfMake.setUrlAccessPolicy(()=>false);pdfMake.setLocalAccessPolicy((path)=>allowedFonts.has(path));pdfMake.setFonts(this.fonts);return pdfMake.createPdf(doc).getBuffer();
  }
}
