...
export class PerformanceService {
  ...
  private present<T extends Record<string,any>>(item:T){return{...item,receivedBytes:Number(item.receivedBytes),labels:item.labels?.map((label:any)=>({...label,receivedBytes:Number(label.receivedBytes)}))};}
}
