...
export class UsersService {
  ...
  async list(organizationId: string, query: ListUsersDto) {
    ...
    if (query.search) userWhere.OR = ['email', 'username', 'firstName', 'lastName'].map(field => ({ [field]: { contains: query.search, mode: 'insensitive' } }));
    const where: Prisma.OrganizationMemberWhereInput = { organizationId, ...(query.role ? { role: query.role } : {}), user: userWhere };
    const [items, total] = await this.prisma.$transaction([
      this.prisma.organizationMember.findMany({ where, select: { role: true, createdAt: true, user: { select: { id: true, email: true, username: true, firstName: true, lastName: true, isActive: true, lastLoginAt: true, createdAt: true } } }, orderBy: { createdAt: 'desc' }, skip: (query.page - 1) * query.pageSize, take: query.pageSize }),
  ...
  async stats(organizationId: string) {
    const memberships = await this.prisma.organizationMember.findMany({ where: { organizationId }, select: { role: true, user: { select: { isActive: true } } } });
    return { total: memberships.length, active: memberships.filter(item => item.user.isActive).length, inactive: memberships.filter(item => !item.user.isActive).length, roles: memberships.reduce<Record<string, number>>((all, item) => ({ ...all, [item.role]: (all[item.role] ?? 0) + 1 }), {}) };
  }

  async create(actor: JwtUser, body: CreateUserDto) {
    const email = body.email.trim().toLowerCase(), username = body.username.trim().toLowerCase();
  ...
  async update(actor: JwtUser, id: string, body: UpdateUserDto) {
    ...
    if (id === actor.sub && body.isActive === false) throw new BadRequestException('Нельзя отключить собственную учётную запись');
    if (membership.role === MembershipRole.ADMIN && ((body.role && body.role !== MembershipRole.ADMIN) || body.isActive === false)) await this.ensureAnotherAdmin(actor.organizationId, id);
    return this.prisma.$transaction(async tx => {
  ...
  async resetPassword(actor: JwtUser, id: string, password: string) {
    await this.member(actor.organizationId, id);
  ...
  private async member(organizationId: string, userId: string) {
    const membership = await this.prisma.organizationMember.findUnique({ where: { organizationId_userId: { organizationId, userId } }, include: { user: true } });
    if (!membership) throw new NotFoundException('Пользователь не найден в организации');
    return membership;
  }
  private async ensureAnotherAdmin(organizationId: string, excludedUserId: string) {
    const count = await this.prisma.organizationMember.count({ where: { organizationId, role: MembershipRole.ADMIN, userId: { not: excludedUserId }, user: { isActive: true } } });
    if (!count) throw new BadRequestException('В организации должен остаться хотя бы один активный администратор');
  }
}
