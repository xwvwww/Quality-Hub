...
export class AutomationService {
  ...
  async createKey(organizationId: string, userId: string, dto: CreateApiKeyDto) {
    ...
    await this.prisma.auditLog.create({ data: { organizationId, userId, action: 'AUTOMATION_KEY_CREATED', entityType: 'AUTOMATION_API_KEY', entityId: created.id, metadata: { name: created.name, projectId: dto.projectId } } });
  ...
  async revoke(organizationId: string, userId: string, id: string) {
    ...
    await this.prisma.$transaction([this.prisma.automationApiKey.update({ where: { id }, data: { revokedAt: new Date() } }), this.prisma.auditLog.create({ data: { organizationId, userId, action: 'AUTOMATION_KEY_REVOKED', entityType: 'AUTOMATION_API_KEY', entityId: id } })]);
    return { success: true };
  }
