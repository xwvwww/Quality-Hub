...
export class ProfileController {
  ...
  @Delete('sessions/:id') async revokeSession(@Req() req: AuthRequest, @Res({ passthrough: true }) res: Response, @Param('id') id: string) { const result=await this.service.revokeSession(req.user.sub,id,req.user.organizationId,req.cookies?.quality_hub_refresh);if(result.current)res.clearCookie('quality_hub_refresh',{path:'/api/auth'});return result; }
